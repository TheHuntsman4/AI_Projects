...
area2=[(576,372),(590,394),(693,386),(669,377)]
in_area_1=set()
in_area_2=set()
while True:
    ret,frame=cap.read()
    ...
    for bbox in idx_bbox:
        ...
        result_leaving=cv2.pointPolygonTest(np.array(area1,np.int32),((x3,y3)),False)
        if result_leaving>0:
            in_area_1.add(id)
        
        result_entering=cv2.pointPolygonTest(np.array(area2,np.int32),((x3,y3)),False)
        if result_entering>0:
            in_area_2.add(id)
...
print(f'The number of vehicles entering the frame: {len(in_area_2)}')